...
class BenchmarkTest:
    ...
    def run_benchmark(self, complexity="simple", num_requests=50, max_workers=5):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(self.benchmark_request, complexity)
    ...
    def compare_benchmarks(self):
        ...
        if "simple" in self.benchmark_results and "complex" in self.benchmark_results:
            ...
            response_time_increase = (
                (complex_result["avg_response_time"] - simple["avg_response_time"])
                / simple["avg_response_time"]
                * 100
            )
            throughput_decrease = (
                (simple["throughput"] - complex_result["throughput"])
                / simple["throughput"]
                * 100
            )

            print(
                f"Response time increase (simple → complex): {response_time_increase:.1f}%"
            )
    ...
    def plot_benchmark_comparison(self, save_path="benchmark_comparison.png"):
        ...
        fig.suptitle(
            "AI Cybersecurity Tool - Benchmark Comparison",
            fontsize=16,
            fontweight="bold",
        )
        ...
        avg_times = [
            self.benchmark_results[c]["avg_response_time"] for c in complexities
        ]
        p95_times = [
            self.benchmark_results[c]["p95_response_time"] for c in complexities
        ]
        ...
        success_rates = [
            self.benchmark_results[c]["success_rate"] for c in complexities
        ]
        ...
        threat_scores = [
            self.benchmark_results[c]["avg_threat_score"] for c in complexities
        ]
