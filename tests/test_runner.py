...
def run_unit_tests():
    ...
    return result.wasSuccessful(), len(result.testsRun), len(result.failures), len(result.errors)
...
def generate_test_report(unit_success, unit_total, unit_failures, unit_errors,
    ...
    print(f"Unit Tests:        {unit_total:3d} tests, {unit_failures:2d} failures, {unit_errors:2d} errors")
    print(f"Integration Tests: {integration_total:3d} tests, {integration_failures:2d} failures, {integration_errors:2d} errors")
    print(f"Performance Tests: {performance_total:3d} tests, {performance_failures:2d} failures, {performance_errors:2d} errors")
    print("-" * 50)
    print(f"Total:             {total_tests:3d} tests, {total_failures:2d} failures, {total_errors:2d} errors")
...
def main():
    ...
    # Run integration tests
    integration_success, integration_total, integration_failures, integration_errors = run_integration_tests()
    
    # Run performance tests
    performance_success, performance_total, performance_failures, performance_errors = run_performance_tests()
    
    # Generate report
    overall_success = generate_test_report(
        unit_success, unit_total, unit_failures, unit_errors,
        integration_success, integration_total, integration_failures, integration_errors,
        performance_success, performance_total, performance_failures, performance_errors
    )
