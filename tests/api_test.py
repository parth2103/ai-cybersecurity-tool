...
class TestAPIEndpoints:
    ...
    def test_predict_endpoint_structure(self, sample_request_data):
        ...
        response = self.session.post(
            f"{self.base_url}/predict", json=sample_request_data
        )
        assert response.status_code == 200
...
class TestAPIPerformance:
    ...
    def test_concurrent_requests(self):
        ...
        def make_request():
            ...
            return self.session.post(f"{self.base_url}/predict", json=data)
    ...
    def test_response_time_consistency(self):
        ...
        for _ in range(5):
            start_time = time.time()
            response = self.session.post(
                f"{self.base_url}/predict", json=self.sample_data
            )
            elapsed_time = time.time() - start_time
        ...
        assert (
            max_time < avg_time * 2
        ), f"Response time inconsistency: max={max_time:.2f}s, avg={avg_time:.2f}s"
