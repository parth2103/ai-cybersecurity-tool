...
class PerformanceTest:
    ...
    def load_test(self, num_requests=100, max_workers=10, attack_type="normal"):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(self.single_request, attack_type) for _ in range(num_requests)]
            
            for i, future in enumerate(as_completed(futures)):
                result = future.result()
                if result['success']:
                    self.results['response_times'].append(result['time'] * 1000)  # Convert to ms
        ...
        metrics = {
            ...
            'avg_response_time': np.mean(self.results['response_times']) if self.results['response_times'] else 0,
            'p50_response_time': np.percentile(self.results['response_times'], 50) if self.results['response_times'] else 0,
            'p95_response_time': np.percentile(self.results['response_times'], 95) if self.results['response_times'] else 0,
            'p99_response_time': np.percentile(self.results['response_times'], 99) if self.results['response_times'] else 0,
            'min_response_time': np.min(self.results['response_times']) if self.results['response_times'] else 0,
            'max_response_time': np.max(self.results['response_times']) if self.results['response_times'] else 0,
            'std_response_time': np.std(self.results['response_times']) if self.results['response_times'] else 0,
            'avg_threat_score': np.mean(threat_scores) if threat_scores else 0,
            'threat_level_distribution': pd.Series(threat_levels).value_counts().to_dict() if threat_levels else {}
        }
    ...
    def stress_test(self, duration_seconds=60, attack_type="normal"):
        ...
        return {
            ...
            'failed_requests': self.results['errors'],
            'success_rate': (request_count - self.results['errors']) / request_count * 100,
            'total_time': total_time,
    ...
    def spike_test(self, base_requests=50, spike_requests=200, max_workers=20):
        ...
        print(f"⚡ Starting spike test: {base_requests} base + {spike_requests} spike requests")
        ...
        return {
            ...
            'performance_degradation': {
                'response_time_increase': (spike_results['avg_response_time'] - baseline_results['avg_response_time']) / baseline_results['avg_response_time'] * 100,
                'throughput_decrease': (baseline_results['throughput'] - spike_results['throughput']) / baseline_results['throughput'] * 100
            }
        }
    ...
    def plot_results(self, save_path='performance_results.png'):
        ...
        fig.suptitle('AI Cybersecurity Tool - Performance Test Results', fontsize=16, fontweight='bold')
        
        # Response time distribution
        axes[0, 0].hist(self.results['response_times'], bins=50, edgecolor='black', alpha=0.7, color='skyblue')
        axes[0, 0].set_title('Response Time Distribution', fontweight='bold')
        axes[0, 0].set_xlabel('Response Time (ms)')
        axes[0, 0].set_ylabel('Frequency')
        axes[0, 0].axvline(np.mean(self.results['response_times']), color='red', linestyle='--', label=f'Mean: {np.mean(self.results["response_times"]):.1f}ms')
        axes[0, 0].legend()
        ...
        axes[0, 1].axhline(np.mean(self.results['response_times']), color='red', linestyle='--', alpha=0.7)
        
        # Percentiles
        percentiles = [50, 75, 90, 95, 99]
        percentile_values = [np.percentile(self.results['response_times'], p) for p in percentiles]
        bars = axes[0, 2].bar([f'P{p}' for p in percentiles], percentile_values, color='orange', alpha=0.7)
        ...
        success_rate = len(self.results['response_times']) / (len(self.results['response_times']) + self.results['errors'])
        ...
        axes[1, 1].boxplot(self.results['response_times'], patch_artist=True, 
...
def main():
    ...
    print("="*60)
    normal_load_results = tester.load_test(num_requests=100, max_workers=10, attack_type="normal")
    all_results['normal_load'] = normal_load_results
    
    # Clear results for next test
    tester.results = {'response_times': [], 'throughput': [], 'errors': 0, 'error_details': [], 'test_timestamps': []}
    
    # 2. Load Test - DDoS Traffic
    print("\n" + "="*60)
    print("TEST 2: LOAD TEST - DDOS TRAFFIC")
    print("="*60)
    ddos_load_results = tester.load_test(num_requests=100, max_workers=10, attack_type="ddos")
    ...
    print("="*60)
    spike_results = tester.spike_test(base_requests=30, spike_requests=100, max_workers=15)
