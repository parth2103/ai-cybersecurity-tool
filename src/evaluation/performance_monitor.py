...
class PerformanceMonitor:
    ...
    def get_performance_summary(self):
        ...
        return {
            ...
            "avg_memory_percent": (
                np.mean(self.memory_usage) if self.memory_usage else 0
            ),
            "throughput_per_second": (
                1000 / np.mean(self.inference_times) if self.inference_times else 0
            ),
        }
    ...
    def check_performance_requirements(self, max_latency_ms=5000):
        ...
        if not meets_requirement:
            print(
                f"⚠️ Performance Warning: Average inference time ({avg_time:.2f}ms) exceeds requirement ({max_latency_ms}ms)"
            )
        else:
            print(
                f"✅ Performance OK: Average inference time ({avg_time:.2f}ms) within requirement"
            )
