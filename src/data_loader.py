...
class CICIDSDataLoader:
    def __init__(self, data_dir=None):
        if data_dir is None:
            ...
            self.data_dir = self.project_root / 'data' / 'cicids2017' / 'MachineLearningCCSV'
        else:
            self.data_dir = Path(data_dir)
        
        if not self.data_dir.exists():
            print(f"Warning: Data directory not found: {self.data_dir}. A synthetic dataset will be generated.")
    ...
    def load_friday_data(self, sample_size=None):
        ...
        portscan_file = self.data_dir / 'Friday-WorkingHours-Afternoon-PortScan.pcap_ISCX.csv'
