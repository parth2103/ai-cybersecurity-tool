...
class SSLEnhancement:
    ...
    def contrastive_loss(
        self, projections1: torch.Tensor, projections2: torch.Tensor
    ) -> torch.Tensor:
        ...
        similarity_matrix = (
            torch.matmul(projections1, projections2.T) / self.temperature
        )
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                for batch_x1, batch_x2 in val_loader:
                    batch_x1, batch_x2 = batch_x1.to(self.device), batch_x2.to(
                        self.device
                    )
...
def create_ssl_enhanced_features(
    original_features: np.ndarray, ssl_encoder_path: str
) -> np.ndarray:
    """
    ...
    logger.info(
        f"Enhanced features shape: {enhanced_features.shape} "
...
if __name__ == "__main__":
    ...
    metrics = ssl.pretrain(
        X, epochs=50, batch_size=128, save_path="models/ssl_encoder.pkl"
    )
