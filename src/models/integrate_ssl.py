...
class SSLIntegratedModel:
    ...
    def __init__(
        self, baseline_model_path: str, ssl_encoder_path: Optional[str] = None
    ):
        """
        Args:
    ...
    def prepare_ssl_encoder(
        ...
        metrics = ssl.pretrain(
            X_unlabeled, epochs=epochs, batch_size=batch_size, save_path=save_path
        )
    ...
    def train_enhanced_model(
        ...
        X_train_enhanced = np.nan_to_num(
            X_train_enhanced, nan=0.0, posinf=1e6, neginf=-1e6
        )
        X_test_enhanced = np.nan_to_num(
            X_test_enhanced, nan=0.0, posinf=1e6, neginf=-1e6
        )
        ...
        results = self._evaluate_models(
            X_test_orig, X_test_enhanced, y_test, training_time, enhanced_model
        )
    ...
    def _evaluate_models(
        ...
        results = {
            ...
            "relative_improvement": ((enhanced_acc - baseline_acc) / baseline_acc)
            * 100,
        ...
        # Confusion matrices
        results["baseline_confusion_matrix"] = confusion_matrix(
            y_test, baseline_pred
        ).tolist()
        results["enhanced_confusion_matrix"] = confusion_matrix(
            y_test, enhanced_pred
        ).tolist()
    ...
    def get_feature_importance_analysis(self) -> Dict[str, Any]:
        ...
        if not self.performance_comparison:
            raise ValueError(
                "No performance comparison available. Train the enhanced model first."
            )
        ...
        analysis = {
            ...
            "baseline_top_importance": baseline_importance[
                baseline_top_features
            ].tolist(),
            "enhanced_top_importance": enhanced_importance[
                enhanced_top_features
            ].tolist(),
            "ssl_feature_contribution": np.mean(
                enhanced_importance[self.baseline_model.n_features_in_ :]
            ),
        }
...
def run_ssl_integration_pipeline(
    ...
    if test_ssl_only:
        return {"ssl_training": ssl_metrics, "status": "ssl_only_test"}

    # Train enhanced model and compare performance
    performance_results = integrated_model.train_enhanced_model(
    ...
    results = {
        ...
        "data_info": {
            ...
            "class_distribution": {
                str(k): int(v)
                for k, v in zip(*np.unique(y_processed, return_counts=True))
            },
        },
    }
...
if __name__ == "__main__":
    ...
    results = run_ssl_integration_pipeline(
        data_sample_size=5000, ssl_epochs=30, test_ssl_only=False
    )
    ...
    print(
        f"Feature Count - Baseline: {perf['baseline_features']}, Enhanced: {perf['enhanced_features']}"
    )
