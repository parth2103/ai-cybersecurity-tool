...
class AlertManager:
    ...
    def create_alert(self, title: str, message: str, severity: AlertSeverity, 
                    source: str, metadata: Optional[Dict] = None) -> Alert:
        """Create a new alert"""
...
class HealthChecker:
    ...
    def add_check(self, name: str, check_func: Callable[[], bool], 
                  error_message: str, severity: AlertSeverity = AlertSeverity.MEDIUM):
        """Add a health check"""
        self.checks[name] = {
            'function': check_func,
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        for name, check in self.checks.items():
            check_status = {
                'healthy': check['last_result'] if check['last_result'] is not None else None,
...
def create_alert(title: str, message: str, severity: AlertSeverity, 
                source: str, metadata: Optional[Dict] = None) -> Alert:
    """Create a new alert"""
    return alert_manager.create_alert(title, message, severity, source, metadata)

def configure_email_alerts(smtp_host: str, smtp_port: int, username: str, 
                          password: str, from_email: str, to_emails: List[str]):
    """Configure email alerts"""
    alert_manager.configure_email(smtp_host, smtp_port, username, password, from_email, to_emails)
