...
class ErrorHandler:
    ...
    def handle_error(self, error: Exception, context: str = "", 
                    severity: ErrorSeverity = ErrorSeverity.MEDIUM,
                    category: ErrorCategory = ErrorCategory.SYSTEM_ERROR) -> Dict[str, Any]:
...
def validate_data(data: Any, data_type: str, required_fields: Optional[list] = None) -> bool:
    """Validate input data"""
    try:
        if data is None:
            raise DataValidationError(f"{data_type} data is None", severity=ErrorSeverity.HIGH)
        ...
        if isinstance(data, dict):
            ...
            if len(data) == 0:
                raise DataValidationError(f"{data_type} data is empty", severity=ErrorSeverity.MEDIUM)
        
        elif isinstance(data, pd.DataFrame):
            if data.empty:
                raise DataValidationError(f"{data_type} DataFrame is empty", severity=ErrorSeverity.MEDIUM)
        ...
        raise DataValidationError(f"Data validation failed: {str(e)}", severity=ErrorSeverity.HIGH)

def validate_model_input(features: Union[dict, np.ndarray, pd.DataFrame], 
                        expected_features: Optional[list] = None) -> bool:
    ...
    try:
        if isinstance(features, dict):
            ...
            if features.size == 0:
                raise DataValidationError("Model input is empty", severity=ErrorSeverity.HIGH)
            
            # Check for NaN values
            if isinstance(features, np.ndarray):
                if np.any(np.isnan(features)):
                    raise DataValidationError("Model input contains NaN values", severity=ErrorSeverity.MEDIUM)
        ...
        raise DataValidationError(f"Model input validation failed: {str(e)}", severity=ErrorSeverity.HIGH)
    ...
    try:
        ...
        if hasattr(model, 'predict_proba'):
            ...
            if predictions is None or predictions.size == 0:
                raise ModelError(f"Model {model_name} returned empty predictions", model_name)
            return predictions
            ...
            raise ModelError(f"Model {model_name} has no prediction methods", model_name)
...
def safe_file_operation(operation: Callable, file_path: Union[str, Path], 
                       operation_name: str = "file operation") -> Any:
    ...
    try:
        ...
        error_handler.handle_error(e, f"{operation_name} on {file_path}", 
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt < max_retries:
                        ...
                        error_handler.handle_error(e, f"Retry failed for {func.__name__}", 
                                                 ErrorSeverity.HIGH, ErrorCategory.SYSTEM_ERROR)
...
def validate_api_request(request_data: dict, required_fields: list) -> bool:
    ...
    try:
        if not isinstance(request_data, dict):
            raise DataValidationError("Request data must be a dictionary", severity=ErrorSeverity.HIGH)
        ...
        raise DataValidationError(f"API request validation failed: {str(e)}", severity=ErrorSeverity.HIGH)

# Convenience functions
def handle_model_error(error: Exception, model_name: str = "unknown"):
    """Handle model-specific errors"""
    return error_handler.handle_error(error, f"Model: {model_name}", 
                                    ErrorSeverity.MEDIUM, ErrorCategory.MODEL_ERROR)

def handle_api_error(error: Exception, endpoint: str = "unknown"):
    """Handle API-specific errors"""
    return error_handler.handle_error(error, f"API: {endpoint}", 
                                    ErrorSeverity.MEDIUM, ErrorCategory.API_ERROR)

def handle_data_error(error: Exception, data_source: str = "unknown"):
    """Handle data-specific errors"""
    return error_handler.handle_error(error, f"Data: {data_source}", 
                                    ErrorSeverity.MEDIUM, ErrorCategory.DATA_VALIDATION)
