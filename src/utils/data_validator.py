...
class DataValidator:
    ...
    def validate_api_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'features' not in request_data:
            raise DataValidationError("Missing 'features' field", severity=ErrorSeverity.HIGH)
        
        features = request_data['features']
        if not isinstance(features, dict):
            raise DataValidationError("Features must be a dictionary", severity=ErrorSeverity.HIGH)
        
        # Validate network features
        try:
            validated_data['network_features'] = self.validate_network_features(features)
    ...
    def prepare_model_input(self, features: Dict[str, Any]) -> np.ndarray:
        ...
        try:
            ...
            raise DataValidationError(
                f"Failed to prepare model input: {str(e)}",
                severity=ErrorSeverity.HIGH
            )
    
    def detect_anomalous_input(self, features: Dict[str, Any]) -> Tuple[bool, List[str]]:
