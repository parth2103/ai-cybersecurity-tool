...
class APIKeyManager:
    ...
    def generate_api_key(self, name: str, permissions: List[str], 
                        rate_limit: int = 100, expires_hours: Optional[int] = None) -> str:
    ...
    def check_rate_limit(self, key: str) -> Tuple[bool, int, int]:
        ...
        self.key_usage[key] = [
            usage_time for usage_time in self.key_usage[key] 
            if usage_time > hour_ago
        ]
        ...
        def decorated_function(*args, **kwargs):
            ...
            if permissions:
                ...
                if not any(perm in user_permissions for perm in permissions):
                    logger.warning(f"Insufficient permissions for API key from {request.remote_addr}")
                    return jsonify({
            ...
            # Check rate limit
            within_limit, current_usage, rate_limit = api_key_manager.check_rate_limit(api_key)
            if not within_limit:
                logger.warning(f"Rate limit exceeded for API key from {request.remote_addr}")
                return jsonify({
