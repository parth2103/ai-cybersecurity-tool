...
class ThreatDatabase:
            ...
            with self.lock:
                ...
                for row in rows:
                    ...
                    if threat["model_predictions"]:
                        threat["model_predictions"] = json.loads(
                            threat["model_predictions"]
                        )
    ...
    def get_alerts(
        self, limit: int = 100, acknowledged: Optional[bool] = None
    ) -> List[Dict]:
    ...
    def get_threat_statistics(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            with self.lock:
                ...
                # Total threats
                cursor.execute(
                    "SELECT COUNT(*) FROM threats WHERE timestamp >= ?", (cutoff,)
                )
                ...
                top_source_ips = [
                    {"ip": row[0], "count": row[1]} for row in cursor.fetchall()
                ]
    ...
    def cleanup_old_data(self, days: int = 30):
        ...
        try:
            with self.lock:
                ...
                logger.info(
                    f"Cleaned up old data: {threats_deleted} threats, {metrics_deleted} metrics, {usage_deleted} API usage records"
                )
